...
parser.add_argument('--data_path', default='', type=str, metavar='PATH', help='path to UCF-101 optical flow data')
...
def main():
    ...
    # Prepare DataLoader
    data_loader = dataloaders.motion.MotionDataloader(
...
class MotionCNN():
    ...
    def webcam_inference(self):
        ...
        nn_output = torch.tensor(np.zeros((1, 101)), dtype=torch.float32).to(self.device)
    ...
    def build_model(self):
        ...
        self.model = resnet101(pretrained=True, channel=self.channel).to(self.device)
