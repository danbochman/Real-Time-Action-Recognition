...
import dataloaders
from dataloaders.spatial import UCF101Splitter
...
parser.add_argument('--data_path', default='', type=str, metavar='PATH', help='path to UCF-101 dataset (default: none)')
...
def main():
    ...
    # Prepare DataLoader
    dataloader = dataloaders.spatial.SpatialDataloader(
    ...
    train_loader, test_loader, test_video = dataloader.run()
    # Model
    model = SpatialCNN(
        device=device,
...
class SpatialCNN(object):
    def __init__(self, device, nb_epochs, lr, batch_size, resume, start_epoch, evaluate, train_loader, test_loader,
                 test_video,
                 demo):
    ...
    def webcam_inference(self):
        ...
        data_handler = UCF101Splitter(os.getcwd() + '/UCF_data_references/', None)
        ...
        while True:
            ...
            frame = transform(frame).view(1, 3, 224, 224).to(self.device)

            # feed the frame to the neural network

            # vote for class every 30 consecutive frames
            if frame_count % 10 == 0:
                nn_output = self.model(frame).to(self.device)
                nn_output = softmax(nn_output)
                nn_output = nn_output.data.to(self.device).numpy()
    ...
    def build_model(self):
        ...
        # build model
        self.model = resnet101(pretrained=True, channel=3).to(self.device)
    ...
    def train_1epoch(self):
        ...
        for i, (data_dict, label) in enumerate(progress):
            ...
            # compute output
            output = Variable(torch.zeros(len(data_dict['img1']), 101).float()).to(self.device)
    ...
    def validate_1epoch(self):
        ...
        for i, (keys, data, label) in enumerate(progress):
            ...
            label = label.to(self.device)
            data_var = Variable(data).to(self.device)
            label_var = Variable(label).to(self.device)
            ...
            preds = output.data.to(self.device).numpy()
    ...
    def frame2_video_level_accuracy(self):
        ...
        return top1, top5, loss.data.to(self.device).numpy()
